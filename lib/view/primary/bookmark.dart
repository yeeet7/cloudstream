      ...
      body: SingleChildScrollView(
        child: Container(
          ...
          child: BookmarksStateStorage.sortType != SortType.custom ? Wrap(
            ...
            runSpacing: 10,
            children: [
              if(BookmarksStateStorage.type == BookmarkType.watching) ...snapshot.watching.sortByType(BookmarksStateStorage.sortType, BookmarksStateStorage.sortDirIsAsc).map<Widget>((e) => Movie(e.value, (Hive.box('config').get('ItemsInRowCount', defaultValue: 3) as double).floor())).toList(),
              if(BookmarksStateStorage.type == BookmarkType.completed) ...snapshot.completed.sortByType(BookmarksStateStorage.sortType, BookmarksStateStorage.sortDirIsAsc).map<Widget>((e) => Movie(e.value, (Hive.box('config').get('ItemsInRowCount', defaultValue: 3) as double).floor())).toList(),
              if(BookmarksStateStorage.type == BookmarkType.planned) ...snapshot.planned.sortByType(BookmarksStateStorage.sortType, BookmarksStateStorage.sortDirIsAsc).map<Widget>((e) => Movie(e.value, (Hive.box('config').get('ItemsInRowCount', defaultValue: 3) as double).floor())).toList(),
              if(BookmarksStateStorage.type == BookmarkType.onHold) ...snapshot.onHold.sortByType(BookmarksStateStorage.sortType, BookmarksStateStorage.sortDirIsAsc).map<Widget>((e) => Movie(e.value, (Hive.box('config').get('ItemsInRowCount', defaultValue: 3) as double).floor())).toList(),
              if(BookmarksStateStorage.type == BookmarkType.dropped) ...snapshot.dropped.sortByType(BookmarksStateStorage.sortType, BookmarksStateStorage.sortDirIsAsc).map<Widget>((e) => Movie(e.value, (Hive.box('config').get('ItemsInRowCount', defaultValue: 3) as double).floor())).toList(),
